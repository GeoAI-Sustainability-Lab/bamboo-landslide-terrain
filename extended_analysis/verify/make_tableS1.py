...
S14=json.load(open('verified_facts_s14.json'))['facts']
...
add('A',1,'main specification (centroid, island-wide controls)',h['OR'],*h['ci'],h['p'],'A')
fe=F['S6.event_fixed_effects']['value']
add('A',2,'event fixed effects, 5 km affected-area cells',fe['fe']['OR'],*fe['fe']['ci'],fe['fe']['p'],'B')
...
for i,(k,lab) in enumerate([('2km','affected area, 2 km grid cells'),('10km','affected area, 10 km grid cells'),
                            ('20km','affected area, 20 km grid cells'),('buffer2km','affected area, 2 km buffer around the polygons'),
                            ('buffer5km','affected area, 5 km buffer around the polygons')],start=6):
...
lab={'centroid':'centroid (main specification)','highest_cell':'highest DEM cell, terrain only',
...
add('B',i,'highest DEM cell, forest type and terrain both re-read',sp['OR'],*sp['ci'],sp['p'],'B')
sc=F['S6.screens']['value']
order=[('exclude_reactivation','exclude cases on scars mapped in 2004 to 2017'),
 ...
 ('exclude_large_ls_potential','exclude large-scale landslide potential areas'),
 ...
 ('reliably_shallow_original_controls','combined screen, original controls'),
 ('reliably_shallow_new_pool','combined screen, new control pool'),
...
add('D',7,'pooled, combined screen, new pool',sc['pooled_reliably_shallow_new_pool']['OR'],*sc['pooled_reliably_shallow_new_pool']['ci'],sc['pooled_reliably_shallow_new_pool']['p'],'A')
tl=F['S6.three_level']['value']['new_pool']
add('D',8,'pure bamboo and mixed stands as separate levels: pure bamboo term',tl['bamboo_x_slope']['OR'],*tl['bamboo_x_slope']['ci'],tl['bamboo_x_slope']['p'],'A')
add('D',9,'pure bamboo and mixed stands as separate levels: mixed-stand term',tl['mixed_x_slope']['OR'],*tl['mixed_x_slope']['ci'],tl['mixed_x_slope']['p'],'A')
for i,r in enumerate(F['S6.per_event']['value']['pure'],start=10):
    EN={'0728豪雨':'28 July 2025 rainfall event','凱米颱風':'Typhoon Gaemi, July 2024'}
...
ecr=F['S5.event_cluster_robust']['value']['reproduced']; add('A',17,'main specification, cluster-robust SE, events as clusters',ecr['OR'],*ecr['ci'],ecr['p'],'A')
gl=S9['S9.glmm_event_random_intercept']['value']; add('A',18,'logistic generalized linear mixed model, event random intercept',gl['interaction_OR'],*gl['ci'],None,'B','posterior 95% interval')
...
add('F',10,'vegetation-index subsample without indices (5,393 cases, 9,148 controls)',gr['base_on_covered_subset']['OR'],*gr['base_on_covered_subset']['ci'],gr['base_on_covered_subset']['p'],'A')
...
add('C',17,'exclude cases inside an earlier 2018-2025 polygon',wr['rule_earlier_polygon']['refit']['OR'],*wr['rule_earlier_polygon']['refit']['ci'],wr['rule_earlier_polygon']['refit']['p'],'A')
...
hs=S14['S14.horn_slope_refit']['value']['horn']; add('F',13,'slope by the Horn method in all slope terms',hs['OR'],*hs['ci'],hs['p'],'A')
...
ax.set_xlabel('Interaction odds ratio per degree of slope, bamboo vs broadleaf',fontsize=9)
...
h=[Line2D([],[],color=grp[g],lw=1.35,marker='o',ms=4.7,label=f"{g}  {t} ({cnt.get(g,0)})") for g,t in
   [('A','Event exposure, rainfall and inference'),('B','Polygon and source-point representation'),
    ('C','Failure-type screens and combined screens'),('D','Stand definition, growth form and event structure'),
...
h+= [Line2D([],[],color='0.3',lw=0,marker='o',ms=4.7,mfc='white',mec='0.3',label='open marker: p ≥ 0.05'),
     Line2D([],[],color='k',lw=1,ls=':',label='main estimate, 1.0341')]
